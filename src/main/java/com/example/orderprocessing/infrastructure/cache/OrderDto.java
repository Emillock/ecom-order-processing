...
/**
 * Flat data-transfer record that mirrors all {@link Order} fields in a form that
 * Jackson can serialize and deserialize without requiring access to the package-private
 * {@link Order} constructor.
 *
 * <p>Used by {@link RedisOrderCache} to convert domain {@link Order} instances to and
 * from the JSON payload stored in Redis. The shape is identical to the REST
 * {@code OrderResponse} so that the cache payload satisfies Requirement 18.
 */
public record OrderDto(
    ...
    public Order toDomain() {
        OrderBuilder builder = new OrderBuilder()
                .id(new OrderId(id))
                .items(items.stream().map(OrderItemDto::toDomain).toList())
                .status(status)
