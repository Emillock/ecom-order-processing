...
- [ ] 2. Implement core domain value objects (immutable, no Spring)
  - [~] 2.1 Create primitive value records: `OrderId`, `Sku`, `IdempotencyKey`, `Money`
  ...
  - [~] 2.2 Create `OrderItem` record with quantity >= 1 invariant
    - Compact constructor rejects quantity < 1
    - _Requirements: 1.2_
  - [~] 2.3 Create `OrderStatus` enum with `isTerminal()` (DELIVERED, CANCELLED, FAILED)
    - _Requirements: 6.4_
  - [~] 2.4 Create `OrderStatusEvent` record (orderId, from, to, at, actor, reason)
    - _Requirements: 6.3, 8.4_
  - [~] 2.5 Write unit tests for value object invariants
...
- [ ] 3. Implement `Order` aggregate with Builder
  - [~] 3.1 Create immutable `Order` class with package-private constructor
  ...
  - [~] 3.2 Create `OrderBuilder` (Builder pattern) enforcing required fields and defaults
...
- [ ] 4. Implement order lifecycle state machine (State pattern)
  - [~] 4.1 Create `OrderState` interface and per-status state classes
  ...
  - [~] 4.2 Create `OrderStateMachine.assertTransition(from, to)` guard
...
- [ ] 5. Implement validation chain (Chain of Responsibility)
  - [~] 5.1 Create `ValidationRule` interface and `ValidationResult` value
    - Each rule returns pass/fail with stable rule id and message
    - _Requirements: 2.1, 14.2_
  - [~] 5.2 Implement individual rules under `domain.validation.rules`
    - `NonEmptyItemsRule`, `PositiveQuantityRule`, `KnownSkuRule`, `IdempotencyRule`
    - _Requirements: 1.2, 2.1, 2.3_
  - [~] 5.3 Implement `ValidationChain` orchestrator
...
- [ ] 6. Implement pricing engine (Strategy + Factory Method)
  - [~] 6.1 Create `PricingStrategy` interface returning a `PriceContribution`
    - LSP: every implementation honors non-negative subtotal/tax/shipping postconditions
    - _Requirements: 3.2, 3.5, 13.4, 14.2_
  - [~] 6.2 Implement strategies under `domain.pricing.strategies`
  ...
  - [~] 6.3 Implement `PricingStrategyChainFactory` (Factory Method)
  ...
  - [~] 6.9 Write unit tests for `PricingEngine` failure path
    - Missing profile triggers FAILED transition with pricing-error reason
    - _Requirements: 3.6_

- [ ] 7. Implement notification dispatcher (Observer)
  - [~] 7.1 Create `OrderEventListener` interface and `NotificationDispatcher` (Subject)
  ...
  - [~] 7.2 Define `NotificationChannel` enum and event payload
...
- [ ] 8. Define domain ports (Dependency Inversion seams)
  - [~] 8.1 Create port interfaces under `domain.port`
  ...
  - [~] 8.2 Define result types: `ReservationResult`, `AuthorizationResult`
...
- [ ] 9. Implement persistence adapter (JPA)
  ...
  - [~] 9.2 Implement `JpaOrderRepository` adapting to `OrderRepository`
...
- [ ] 10. Implement cache adapter (Decorator + Redis)
  - [~] 10.1 Configure `RedisTemplate` and TTL via `CacheConfig`
    - Key schema: `order:v1:{orderId}`; TTL configurable via `app.cache.order.ttl` (default 5m)
    - _Requirements: 10.1, 10.2_
  - [~] 10.2 Implement `RedisOrderCache` adapting to `OrderCachePort`
  ...
  - [~] 10.3 Implement `CachingOrderRepository` (Decorator over `JpaOrderRepository`)
  ...
  - [~] 10.4 Write property test for cache-population idempotence
    - **Property 7: Repeated `put(orderId, orderState)` with identical state leaves the cache observably unchanged**
    - **Validates: Requirements 19.1**
  - [~] 10.5 Write `@DataRedisTest` for `RedisOrderCache` round-trip
...
- [ ] 11. Implement order serialization with round-trip property
  ...
  - [~] 11.3 Write unit test for malformed JSON rejection
    - Missing required field and bad type produce a descriptive error and no partial Order
    - _Requirements: 18.4_

- [ ] 12. Implement inventory adapter (Adapter + Resilience4j)
  - [~] 12.1 Implement `HttpInventoryAdapter` adapting to `InventoryPort`
    - Translates external/mock HTTP/JSON to `ReservationResult`; supports `reserve` and `release`
    - _Requirements: 4.1, 4.2, 4.3, 7.2, 13.5_
  - [~] 12.2 Annotate adapter calls with `@CircuitBreaker(name="inventory")`
  ...
  - [~] 12.3 Write unit tests for `HttpInventoryAdapter` translations
    - Mock HTTP client; cover success, out-of-stock, transport failure
    - _Requirements: 4.1, 4.2, 4.3_

- [ ] 13. Implement payment adapter (Adapter + Resilience4j)
  - [~] 13.1 Implement `HttpPaymentAdapter` adapting to `PaymentPort`
    - `authorize(orderId, grandTotal)` and `voidAuthorization(orderId)` against external/mock
    - _Requirements: 5.1, 5.2, 5.3, 13.5_
  - [~] 13.2 Annotate adapter calls with `@CircuitBreaker(name="payment")`
  ...
  - [~] 13.3 Write unit tests for `HttpPaymentAdapter` translations
    - Cover authorization success, decline reason, transport failure
    - _Requirements: 5.1, 5.2, 5.3_

- [ ] 14. Configure Resilience4j circuit breakers and shared config
  - [~] 14.1 Add Resilience4j instance configs in `application.yml`
    - Instances: `inventory`, `payment`, `cache` per design (window, thresholds, durations)
    - _Requirements: 12.2, 12.4, 12.5, 12.6_
  - [~] 14.2 Wire breaker state observation for cache adapter
    - `OrderCachePort.isAvailable()` returns false while `cache` breaker is OPEN
    - _Requirements: 10.3, 12.3_
  - [~] 14.3 Write integration test for breaker open → half-open → closed transitions
    - Use Resilience4j test support; simulate failures past threshold and verify state transitions
    - _Requirements: 12.2, 12.4, 12.5, 12.6_

- [ ] 15. Configure virtual-thread executors and per-Order concurrency
  - [~] 15.1 Enable virtual threads for HTTP and create `pipelineExecutor` bean
  ...
  - [~] 15.2 Implement `OrderLockRegistry` with per-`OrderId` `ReentrantLock` stripes
    - `withLock(orderId, body)` acquires/releases lock; reads do not take the lock
    - _Requirements: 11.3, 11.4_
  - [~] 15.3 Write property test for per-Order serialization invariant
...
- [ ] 17. Implement application orchestration (Template Method + Command + Facade)
  - [~] 17.1 Implement `OrderProcessingPipeline` (Template Method)
  ...
  - [~] 17.2 Implement command classes under `application.command`
  ...
  - [~] 17.3 Implement `OrderService` Facade
  ...
  - [~] 17.4 Write unit tests for `OrderService` with mocked ports
...
- [ ] 18. Implement REST API (controller, DTOs, error handler)
  - [~] 18.1 Create request/response DTOs under `api.dto`
  ...
  - [~] 18.2 Implement `OrderController` with endpoints from design
  ...
  - [~] 18.3 Implement `GlobalExceptionHandler` with stable error codes
  ...
  - [~] 18.4 Write `@WebMvcTest` slice tests for `OrderController`
...
- [ ] 19. Implement observability
  - [~] 19.1 Add structured log emission on every accepted state transition
  ...
  - [~] 19.2 Configure Actuator health and metrics
  ...
  - [~] 19.3 Write unit tests for the structured-log appender contract
    - Capture logs and assert required fields are present for every transition
    - _Requirements: 16.1_

- [ ] 20. Wire end-to-end integration tests
  - [~] 20.1 Implement integration test covering full order lifecycle
  ...
  - [~] 20.2 Implement integration test for circuit-breaker open path
  ...
  - [~] 20.3 Implement integration test for cache-degraded path
...
- [ ] 21. Author documentation deliverables
  - [~] 21.1 Write repository `README.md`
    - Purpose, build/run instructions (`mvn clean verify`, `mvn spring-boot:run`), configuration keys, API summary table
    - _Requirements: 17.1_
  - [~] 21.2 Write `docs/PATTERNS.md` (Pattern Inventory)
  ...
  - [~] 21.3 Write `docs/REFACTORING_CASE_STUDY.md`
    ...
    - _Requirements: 17.3_
  - [~] 21.4 Add `docs/ARCHITECTURE.md` with Mermaid component diagram
...
- [~] 22. Final checkpoint - Ensure all tests pass and coverage thresholds hold
